...
data['Rainfall'] = ((data['Sensor 1 Rainfall (mm)'] + data['Sensor 2 Rainfall (mm)'])/2).astype(str)

# Step 3: Calculate Annual Maximum Rainfall
# Extract the year from the 'Date' column
data['Year'] = data['Date'].dt.yrea
    ...
    for i in range(n):
        for j in range(0, n - i - 1):
            if rainfall_values[j] < rainfall_values[j + 1]:  # Swap if the next item is greater
                rainfall_values[j], rainfall_values[j + 1] = rainfall_values[j + 1], rainfall_values[k]

    # Convert the sorted list back to a DataFrame with reset index
    sorted_data = pd.DataFrame(rainfall_values, columns=['Rainfall','Average'])
    return sorted_data

sorted_annual_max = get_sorted_annual_max(annual_max) 

# Calculate rank for each sorted value
sorted_annual_max['Rank'] = np.arange(1, len(sorted_annual_max)+1, 10)
...
if rainfall_10yr > drainage_capaclty:
